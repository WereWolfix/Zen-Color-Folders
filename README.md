...
had before you started picking). This works over page content too — e.g.
you can eyedrop a color straight off a YouTube page, not just the browser
chrome — see below for why that needed extra work.
...
builds its own screenshot instead, using two layers:
1. `drawWindow()` — a method only available to privileged/chrome JS —
   rasterizes the browser's own UI (toolbars, sidebar, tab strip, etc.).
2. `browser.drawSnapshot()` — the API built specifically for capturing
   out-of-process tab content, the same mechanism behind Firefox's own
   screenshot tooling — separately captures the selected tab's actual
   page content and composites it on top, at the right position.

That second layer is the important part: under Fission (Firefox's
site-isolation architecture), a web page like YouTube runs in its own
content process, completely separate from the browser chrome process this
script runs in. `drawWindow()` alone can only rasterize same-process
rendering, so without step 2 the page area would come back blank — which
is exactly the bug in an earlier version of this mod. `drawSnapshot()` is
the API designed to cross that process boundary safely, so the merged
canvas ends up with real page pixels, not a blank rectangle where the tab
should be.

**Scope:** this can only see pixels within the current browser window
(its own UI, plus the *selected* tab's page content) — it can't reach
outside the browser process to your OS desktop or other applications, and
if you're using split-view with multiple tabs visible at once, only the
currently-selected one is captured for now (the others would still show
as blank, same underlying limitation as before, just narrower). Let me
know if split-view coverage matters to you and I can extend it to loop
over all visible browsers instead of just the selected one.
...
- The eyedropper only sees the current browser window (see "Scope" above)
  — it cannot sample colors from other applications on your desktop, and
  in split-view it only captures the selected tab's page content, not
  every visible tab.
